...
class PolyMixerReverseCoherenceTest(unittest.TestCase):
    """Regression for the "reverse decided independently per channel/band" bug (see the twin test
    in tests/test_quantized_mixer.py): a stream's multi-band ``channels`` with
    ``0 < reverse_prob < 1`` must reverse the grain as a whole, never one band only."""
    ...
    def test_reverse_decision_drawn_once_per_grain_not_per_channel(self):
        ...
        cfg = AutoMixerConfig(src, [0, 400], sample_length=100, mode="poly",
                              streams=[{"ratio": 4}], reverse_prob=0.5, seed=7)
        calls = []
        ...
        with patch("automixer.mixers.poly_mixer.maybe_reverse", side_effect=spy):
            PolyphonicAutoMixer()._create_grain(cfg, onsets=[0], grain_len=100,
                                                 channels=channels, candidates=[0])
